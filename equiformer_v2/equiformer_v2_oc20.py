...
from .so3 import CoefficientMappingModule, SO3_Embedding, SO3_Grid, SO3_Rotation, SO3_LinearV2
...
_AVG_DEGREE = 10.268862339898828
_AVG_NUM_NODES = 11.555970149253731
    ...
    def forward(self, data):
        ...
        for i in range(self.num_resolutions):
            if self.num_resolutions == 1:
                x.embedding[:, offset_res, :] = self.sphere_embedding(atomic_numbers)  # 原子编码种类可能过多
            else:
        ...
        for i in range(self.num_layers):
            x = self.blocks[i](x, atomic_numbers, edge_distance, edge_index, batch=data.batch)  # SO3_Embedding  # for GraphDropPath
